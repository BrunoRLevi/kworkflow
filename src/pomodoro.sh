...
function show_tags()
{
  ...
  tags=$(select_from 'tag' '"id" AS "ID", "name" AS "Name"' 'condition_array' "$flag" '.mode column' 'id')
...
function fetch_last_pomodoro_session()
{
  ...
  last_pomodoro_session=$(select_from 'pomodoro_report' '"duration","tag_name","description"' '' "$flag" '' 'date DESC, time DESC LIMIT 1')
